...
func int DIA_Bengar_BALTHASARDARFAUFWEIDE_Condition()
{
	if(Npc_KnowsInfo(other,DIA_Bengar_BALTHASAR))
	{
		...
		{
			DIA_Bengar_BALTHASARDARFAUFWEIDE.description = "The militia are gone, and Balthasar can use your land again.";
			return TRUE;
		}
...
func void DIA_Bengar_ALLEIN_Info()
{
	...
	if((Malak_isAlive_Kap3 == TRUE) && (Npc_GetDistToWP(Malak,"FARM3") >= 3000))
	{
		...
		if(!Npc_KnowsInfo(other,DIA_Malak_FLEEFROMPASS))
		{
			...
			B_LogNextEntry(TOPIC_BengarMALAK,"Bengar's alone on his farm. His pal Malak has gone and taken all the others with him. Bengar thinks he's fled to the mountains.");
		}
		else
		{
			B_LogNextEntry(TOPIC_BengarMALAK,"Bengar's alone on his farm. His pal Malak has gone and taken all the others with him");
		};
	}
	else
	{
