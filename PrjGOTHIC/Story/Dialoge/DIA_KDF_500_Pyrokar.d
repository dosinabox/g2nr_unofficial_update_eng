...
func void DIA_Pyrokar_Lernen_Info()
{
	...
	if(!Npc_KnowsInfo(other,DIA_Parlan_MAGE))
	{
		...
		B_LogEntry(Topic_KlosterTeacher,"Master Parlan will ordain me to the first Circles of Magic and show me new magic formulas.");
	};
...
instance DIA_Pyrokar_PERM_nonKDF(C_Info)
{
	...
	permanent = TRUE;
	description = "What about a little blessing? I could surely use it.";
};
...
func void DIA_Pyrokar_BACKFROMOW_Info()
{
	...
	if (hero.guild == GIL_KDF)
	{
		...
		if(!Npc_IsDead(Karras))
		{
			...
			Log_AddEntry(TOPIC_DEMENTOREN,"Pyrokar has instructed Karras to deal with the threat of the Seekers.");
		};
	};
...
//	Pedro.flags = 0;
	Pedro_Traitor = TRUE;
	B_LogEntries(TOPIC_INNOSEYE,"Great. I might have known. I'm too late, those numskulls at the monastery let some novice steal the Eye from them, and now I can go running after the traitor Pedro and just hope he doesn't go and sell it to someone.");
	Log_CreateTopic(TOPIC_TraitorPedro,LOG_MISSION);
	Log_SetTopicStatus(TOPIC_TraitorPedro,LOG_Running);
	B_LogNextEntry(TOPIC_TraitorPedro,"The traitor Pedro stole the Eye of Innos from the monastery. I reckon the Fire Magicians would be only too happy to get their hands on him.");
};
...
func void DIA_Pyrokar_BUCHDERBESSENEN_Info()
{
	...
	if(!Npc_IsDead(Karras) && (hero.guild == GIL_KDF))
	{
		...
		Log_AddEntry(TOPIC_DEMENTOREN,"Karras is supposed to help me find a protection against the Seekers' mental assaults.");
	};
};
...
instance DIA_Pyrokar_SCOBSESSED(C_Info)
{
	...
	permanent = TRUE;
	description = "I think I am possessed. Can you heal me?";
};
