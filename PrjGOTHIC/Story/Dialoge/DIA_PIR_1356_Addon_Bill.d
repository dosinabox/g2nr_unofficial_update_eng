...
func void DIA_Addon_Bill_FoundFriends_Info()
{
	...
	if(Npc_KnowsInfo(other,DIA_Addon_Fisk_Lieferung))
	{
		...
		{
			B_LogEntry(TOPIC_Addon_KillJuan,"A fellow named Juan stole the delivery (a package of lock picks) from Fisk, breaking the deal between bandits and pirates. He is hiding somewhere in the swamp. He may be responsible for the death of Angus and Hank.");
		};
	};
	if(SC_Knows_JuanMurderedAngus == TRUE)
	{
		B_LogEntry(TOPIC_Addon_KillJuan,"Tom the bandit was dealing with Angus and Hank, right before Juan attacked them.");
	};
};
...
func void DIA_Addon_Bill_TeachPlayer_Info()
{
	...
	B_LogEntry(Topic_Addon_PIR_Teacher,"Bill can teach me how to pick pockets.");
};
