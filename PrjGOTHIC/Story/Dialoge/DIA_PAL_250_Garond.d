...
func void DIA_Garond_NeedProof_Info()
{
	...
	B_LogEntries(Topic_MISOLDWORLD,"Before Commander Garond sends me back, he wants me to seek out the three scraper troops and tell him how much ore they have stacked away.");
	Log_CreateTopic(TOPIC_ScoutMine,LOG_MISSION);
	Log_SetTopicStatus(TOPIC_ScoutMine,LOG_Running);
	B_LogNextEntry(TOPIC_ScoutMine,"Commander Garond has given me an assignment. He's sent three troops of scrapers out to look for magic ore. They haven't brought any back so far.");
	Log_AddEntry(TOPIC_ScoutMine,"I've got to find the three scraper troops and find out how much ore they've got stacked away.");
	if(Npc_KnowsInfo(other,DIA_DiegoOw_Beweise))
	{
		Log_AddEntry(TOPIC_ScoutMine,"Diego transported FOUR crates of Silvestro's ore to safety.");
	};
};
...
func void DIA_Garond_OricExperte_Info()
{
	...
	if(!Npc_IsDead(Oric))
	{
		...
		Log_AddEntry(TOPIC_DRACHENJAGD,"Garond's strategic officer Oric may have some useful information for me.");
	}
	else
	{
		AI_Output(self,other,"DIA_Garond_DragonPlettBericht_10_10");	//Luckily not. For the moment they're holding off.
